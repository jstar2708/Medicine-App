...
import com.example.medicine.auth.view_models.RegisterCustomerViewModel
import com.example.medicine.databinding.ActivityRegisterCustomerBinding
...
class RegisterCustomerActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
    ...
    private fun onRegisterButtonClicked(){
        if(binding.addressEditText.editText?.text.isNullOrEmpty() || binding.emailEditText.editText?.text.isNullOrEmpty()
            ...
            || binding.phoneEditText.editText?.text.isNullOrEmpty() || binding.phoneEditText.editText?.text?.length != 10
            || binding.spinner.selectedItem.toString() == "Select state" || binding.pinCodeEditText.editText?.text?.length != 6
            || binding.passwordEditText.editText?.text?.length != 8 || binding.spinnerCity.selectedItem == "Select your city"){
