...
import com.example.medicine.auth.view_models.RegisterChemistViewModel
import com.example.medicine.chemist.HomeActivity
import com.example.medicine.databinding.ActivityRegisterChemistBinding
import com.example.medicine.model.Chemist
import com.google.android.material.snackbar.Snackbar

class RegisterChemistActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {

    private lateinit var binding: ActivityRegisterChemistBinding
    private lateinit var registerChemistViewModel: RegisterChemistViewModel
    ...
    private val contract: ActivityResultLauncher<Intent> = (this).registerForActivityResult(
        ...
        if(isPhoneNumberVerified == "T"){
            registerChemistViewModel.registerChemist(chemist, true)
        }
        else{
            registerChemistViewModel.registerChemist(chemist, false)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityRegisterChemistBinding.inflate(layoutInflater)
        ...
        registerChemistViewModel = ViewModelProvider(this, ViewModelProvider.AndroidViewModelFactory(application))[RegisterChemistViewModel::class.java]

        registerChemistViewModel.toast.observe(this, Observer{
            if(it != null){
                handleToast(it)
            }
        })

        registerChemistViewModel.action.observe(this, Observer {
            if(it != null){
                handleAction()
            }
        })
    ...
    private fun onRegisterButtonClicked() {
        if (binding.addressEditText.editText?.text.isNullOrEmpty() || binding.emailEditText.editText?.text.isNullOrEmpty()
            ...
            || binding.spinnerCity.selectedItem == "Select your city" || binding.uniqueIdEditText.editText?.text.isNullOrEmpty()
        ) {

            Snackbar.make(binding.root, "Fill all details correctly", Snackbar.LENGTH_LONG).show()
            progressDialog.dismiss()
        } else {
            chemist.setAddress(binding.addressEditText.editText?.text.toString())
            chemist.setEmail(binding.emailEditText.editText?.text.toString())
            chemist.setOwnerName(binding.ownerNameEditText.editText?.text.toString())
            chemist.setPassword(binding.passwordEditText.editText?.text.toString())
            chemist.setPhoneNumber(binding.phoneEditText.editText?.text.toString().toLong())
            chemist.setPinCode(binding.pinCodeEditText.editText?.text.toString().toInt())
            chemist.setState(binding.spinner.selectedItem.toString())
            chemist.setShopName(binding.companyNameEditText.editText?.text.toString())
            chemist.setCity(binding.spinnerCity.selectedItem.toString())
            chemist.setUniqueId(binding.uniqueIdEditText.editText?.text.toString())
            val intent = Intent(this, OtpActivity::class.java)
            intent.putExtra("phone", chemist.getPhoneNumber().toString())
